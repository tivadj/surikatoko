...
def FindRelativeMotion(xs1_meter, xs2_meter, debug, la_engine='scipy', conceal_lost_precision=True):
...
class PointsWorld:
    def __init__(self):
        ...
        self.multi_view_factorizer = None
    ...
    def LocateCamAndMap_MultiViewFactorization(self, on_pnt_corresp, debug):
        ...
        # on failure to find [R,T] the head_ind doesn't change
        suc = self.multi_view_factorizer.IntegrateNewFrame(self.framei_from_world_RT_list, self.world_pnts, debug=debug)
        if not suc:
            return
        with self.visual_host.cameras_lock:
...
class ReconstructDemo:
    ...
    def WorkerWalkThroughVirtualWorldCrystalGrid(self, continue_lock, cameras_lock, main_args):
        ...
        min_reproj_err = 1e-4
        la_engine = "scipy"  # default="scipy", ["opencv", "scipy"]
        conceal_lost_precision = True

        from suriko.multi_view_factorization import MultiViewIterativeFactorizer
        def FindRelativeRT(xs1_meter, xs2_meter):
            return FindRelativeMotion(xs1_meter, xs2_meter, debug, la_engine=la_engine, conceal_lost_precision=conceal_lost_precision)
        def OnAdjustWorld(sender):
            optimize_structure = "batch-refine"
            if optimize_structure == "batch-refine":
                sender.LocateCamAndMapBatchRefine(debug)
            elif optimize_structure == "bundle-adjustment":
                ba = BundleAdjustmentKanatani(debug=debug)
                ba.min_err_change_rel = 0.01
                converged, err_msg = ba.ComputeInplace(sender.points_life, sender.cam_mat_pixel_from_meter, sender.world_pnts, sender.framei_from_world_RT_list)
                print("BundleAdjustmentKanatani converged={} err_msg={}".format(converged, err_msg))
                return converged
            return False

        factorizer = MultiViewIterativeFactorizer(el_type, track_sys.points_life, None, track_sys.world_pnts, track_sys.framei_from_world_RT_list,
                                                  FindRelativeRT, min_reproj_err=min_reproj_err, adjust_world_fun=OnAdjustWorld)
        factorizer.use_mpmath = main_args.mpmath
        factorizer.la_engine = la_engine
        factorizer.conceal_lost_precision = conceal_lost_precision
        factorizer.ground_truth_relative_motion = track_sys.ground_truth_relative_motion
        factorizer.ground_truth_map_pnt_pos = track_sys.ground_truth_map_pnt_pos
        track_sys.multi_view_factorizer = factorizer

        def OnCamMatLazyInitialized(cam_mat_pixel_from_meter):
            track_sys.SetCamMat(cam_mat_pixel_from_meter)
            factorizer.cam_mat_pixel_from_meter = cam_mat_pixel_from_meter
        test_data_gen.CamMatChanged(OnCamMatLazyInitialized)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--job", help="specify worker thread to run", type=int, default=61)
