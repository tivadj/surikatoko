...
void DavisonMonoSlam::PredictEstimVars(EigenDynVec* predicted_estim_vars, EigenDynMat* predicted_estim_vars_covar) const
{
    ...
    PredictCameraMotionByKinematicModel(Span(estim_vars_, kCamStateComps), new_cam);
    ...
    if (debug_F_G_derivatives)
    {
        ...
        FiniteDiff_cam_state_by_cam_state(Span(estim_vars_, kCamStateComps), kFiniteDiffEpsDebug, &finite_diff_F);
    ...
    std::optional<Scalar> pnt_inv_dist_gt)
{
    ...
    if (kSurikoDebug)
    {
        auto Pnew_bottom_left_debug = Pnew_bottom_left.eval();
    ...
    Eigen::Matrix<Scalar, kSalientPointComps, Eigen::Dynamic>* sal_pnt_to_other_covar)
{
    ...
    // the bottom left corner of Pnew
    sal_pnt_to_other_covar->resize(Eigen::NoChange, take_estim_vars_count);
    sal_pnt_to_other_covar->noalias() = sal_pnt_by_cam * estim_vars_covar_.topLeftCorner<kCamPQ, Eigen::Dynamic>(kEucl3 + kQuat4, take_estim_vars_count);
...
auto DavisonMonoSlam::GetFilterState(FilterStageType filter_stage) const
